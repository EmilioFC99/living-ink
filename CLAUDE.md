...
- **`pipeline.py` does heavy work at import time.** Module load calls `load_yaml_config()`, creates every data directory, and exports module-level globals (`yaml_config`, `ACTIVE_DESTINATIONS`, `DATA_DIR`, `WHITE_DIR`, …). Tests must patch these globals or pass explicit `config_path`/`destinations` to `SyncPipeline`; importing the module with a different env won't re-read config.

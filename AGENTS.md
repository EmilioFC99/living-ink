...
**A command owns its loop, not its exit code.** `SyncCommand.execute_sync()` returns whether one sync worked; `run()` turns that into an exit code and `WatchCommand` re-runs it on a timer instead. `watch` registers every sync option by delegating to `SyncCommand.register_args`, rides out failed cycles, and stops only on a configuration error or Ctrl+C — a background daemon or supervisor runs it directly rather than wrapping `sync` in a shell loop. `info` is the one read-only surface; the destructive halves live in `config` → Advanced.

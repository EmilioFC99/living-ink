"""Where Living Ink's files live.

Every path the tool reads or writes outside a vault is derived here: the
config file, the data directory, the logs, and the credentials directory. The
repo is stateless, so none of these resolve into the checkout unless the user
explicitly points them there.
"""
...
def credentials_dir(config_path: Optional[Path] = None) -> Path:
    """Locate the directory holding stored credentials.

    Derived from the *resolved config path* rather than from ``Path.home()``,
    for two reasons. A test that redirects the config has to be able to
    redirect the secrets with it, or "the repo is stateless" is a rule nothing
    can check. And a user who points ``LIVING_INK_CONFIG`` at a second profile
    gets that profile's credentials, not the default profile's — which is what
    having two configs was for.

    Args:
        config_path: The resolved ``config.yml`` path. Defaults to whatever
            :func:`get_config_path` resolves for this process.

    Returns:
        Path to the credentials directory. May not exist yet; writing a
        credential creates it.
    """
